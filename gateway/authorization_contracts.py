...
class ExternalPdpDecisionResult:
    ...
    by the PDP server.
    ``consistency='strongest'`` records the
    audited outbound request mode. ``cache_used=False`` records local client
    and application-cache behavior, not a server attestation.
    """
