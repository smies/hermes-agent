...
function dependencyTreeEntries(root, relative = '') {
  ...
  const entries = [];
  for (const name of readdirSync(directory).sort()) {
    // Runtime install freshness marker written by the Python adapter.  It is
    // non-executable metadata outside npm's installed graph and is the sole
    // deliberate exclusion from the closure.
    if (!relative && name === '.hermes-pkg-hash') continue;
