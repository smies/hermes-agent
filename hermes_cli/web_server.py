...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Hermes through the bundled WhatsApp bridge with offline phone-number-code provisioning.",
...
async def start_whatsapp_onboarding(body: WhatsAppOnboardingStart):
    raise HTTPException(
        status_code=410,
        detail=(
            "WhatsApp authentication is available only through "
...
async def get_whatsapp_onboarding_status(pairing_id: str):
    raise HTTPException(
        status_code=410,
        detail="WhatsApp browser authentication is unavailable.",
    )
