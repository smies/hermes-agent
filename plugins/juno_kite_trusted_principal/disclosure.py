    ...
    return {
        ...
        "output_tier_rule": {
            ...
            DOCUMENT_DESCRIPTOR: (
                ...
                "schema offers, then the exact kite_personal_files_read read of the "
                "file it returned; and (b) the exact Gmail message read, then "
                "kite_gmail_attachment_extract. One source returning nothing means "
                "only that the document is not there -- it is never grounds to "
                "report the document missing while the other source is unsearched, "
                "and a local file will never appear in an email search. "
                "Step two: read the document_preview the reader returns before "
                "committing. It is text taken from the artifact itself, on this "
                "machine, and it is the evidence of what the file actually is -- "
                "a name like a scanner default or \"photo\" identifies nothing, "
                "and the surrounding email is not evidence of what an image "
                "contains. Confirm the preview matches the document that was "
                "asked for, not merely the topic: a British passport and an "
                "Irish one are both passports. If it does not match, do not "
                "release it -- read the next candidate instead, and if none "
                "match, say which candidates you found by name and what each "
                "turned out to be. An empty preview means the contents could "
                "not be read, so the candidate is unidentified: treat that as a "
                "reason to keep looking, not to proceed. Release the wrong "
                "document and it is disclosed and cannot be recalled, so ask "
                "rather than guess. When the preview confirms it, reply with only the JSON "
                "object {\"capability_id\": \"<one effective capability id>\"} and "
                "no other text. Returning that JSON without having run step one "
