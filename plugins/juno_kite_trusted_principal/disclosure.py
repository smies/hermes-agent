    ...
    return {
        ...
        "output_tier_rule": {
            ...
            DOCUMENT_DESCRIPTOR: (
                ...
                "Step two: check the document_name the reader returns before "
                "committing. It is the artifact's own name, and it is the only "
                "evidence you get about what the file actually is -- you cannot "
                "see inside it. If it does not identify the document that was "
                "asked for, say plainly which candidates you found, by name, and "
                "what is ambiguous about them; a name like a scanner default or "
                "\"photo\" identifies nothing, and the surrounding email is not "
                "evidence of what an image contains. Release the wrong document "
                "and it is disclosed and cannot be recalled, so ask rather than "
                "guess. When the name does identify it, reply with only the JSON "
